...
exports.migrationDecision = function (assets, nodes, migratioanalysis) {
    ...
    for (var nodeIndex = 0; nodeIndex < nodes.length; nodeIndex++) {
        ...
        if (node.ID != migratioanalysis.currentNode && node.runningStatus == 'operational' && node.resourceStatus.RAM_Available_GB > 3)
    ...
    var candidateNodes = candidates.map(item=>item.resourceStatus);
